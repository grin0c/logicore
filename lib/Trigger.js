...
class Trigger {
  ...
  async calculateCondition (action) {
  ...
  async getPatch(action) {
    return this.patch(action, action.getFreshestInstance());
  }

  async getSubactions(action) {
    return this.subactions(action, action.getFreshestInstance());
  }
};
