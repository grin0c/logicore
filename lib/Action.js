...
const ACTION_SCHEMA = {
  properties: {
    ...
    type: {
      type: "integer",
      enum: Object.values(ACTION_STATUS.TYPE),
      default: null,
    ...
    status: {
      type: "integer",
      enum: Object.values(ACTION_STATUS.CREATED),
...
module.exports = class Action {
  ...
  async populateWithOld(core) {
    ...
    try {
      if (this.type === ACTION_TYPE.CREATE) { return; }

      this.dataOld = this.type === ACTION_TYPE.UPDATE
        ? await db.findOne(this.schemaKey, { id: this.instanceId })
        : await db.findOne(this.schemaKey, this.instanceFilter);

      this.dataDiff = Utils.getPatchDiff(this.data, this.dataOld, db.schemas[this.schemaKey]);
    } catch(err) {
      await logger.logError(this, Event.EVENT_STAGE.POPULATING, {}, err);
      throw err;
    }
  ...
  /**
   * If there are new changed in patch, merges them into dataDiffPrepatched and returns true
