...
class Action {
  ...
  async _prepatchingFilterTriggers(core, depth, hashOfPerformed) {
    ...
    return Promise.filter(triggers[Trigger.TRIGGER_TYPE.PREPATCH][this.schemaKey], async (trigger) => {
  ...
  async perform(core) {
    ...
    try {
      switch (this.type) {
        case ACTION_TYPE.INSERT:
          this.dataResult = await db.insert(this.schemaKey, values);
          break;
        case ACTION_TYPE.UPDATE:
          this.dataResult = await db.update(this.schemaKey, this.instanceId, values);
          break;
        case ACTION_TYPE.UPSERT:
          this.dataResult = await db.upsert(this.schemaKey, this.instanceFilter, values);
          break;
      }
    } catch(err) {
      await logger.logError(
